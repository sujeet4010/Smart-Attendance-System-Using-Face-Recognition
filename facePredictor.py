...
class FacePredictor():
    def __init__(self):
        try:
            ...
            # Attendance log in Excel
            self.attendance_file = "attendance.xlsx"
            self.attendance_sheet = None
            self.initialize_excel()

            # Predefined list of expected names
            self.expected_names = ["Sujeet", "Aditya", "Vishal", "Vaibhav","Saurabh"]
            # # Tracker params
            self.trackers = []
            self.texts = []
    ...
    def initialize_excel(self):
        ...
        try:
            ...
            try:
                ...
                self.attendance_sheet = self.workbook.active
            except FileNotFoundError:
                self.workbook = openpyxl.Workbook()
                self.attendance_sheet = self.workbook.active
                self.attendance_sheet.append(['Name', 'Date', 'Status'])  # Add headers if new file

            print("Attendance file initialized successfully!")
        except Exception as e:
            print(f"Error initializing Excel file: {e}")
    def save_attendance_to_excel(self, name, status="Present"):
        """ Save the attendance to the Excel file """
        try:
            # Record current date and time
            date = datetime.now().strftime("%Y-%m-%d %H:%M:%S")

            # Add the name, date, and status to the sheet
            self.attendance_sheet.append([name, date, status])
    ...
    def detectFace(self):
        ...
        while True:
                ...
                if len(bboxes) != 0:
                    ...
                    for bboxe in bboxes:
                        ...
                        if cos_similarity < cosine_threshold and proba > proba_threshold:
                            ...
                            self.save_attendance_to_excel(name, status="Present")

                            # Add recognized name to the set
                            recognized_names.add(name)
                        # Start tracking
                        tracker = dlib.correlation_tracker()
                        # Convert bbox coordinates to native Python integers
            ...
            if key == ord("q"):
                break
         # Mark Absent for those who were not recognized
        for name in self.expected_names:
            if name not in recognized_names:
                self.save_attendance_to_excel(name, status="Absent")
        cap.release()
